...
<body>
  ...
  const handleCopy = () => {
    navigator.clipboard.writeText(compileHTML());
    setCopied(true);
    setTimeout(() => setCopied(false), 2000);
  };

  const handleDownload = () => {
    const blob = new Blob([compileHTML()], { type: 'text/html' });
  ...
  return (
    <div className="bg-white border border-gray-200 rounded-2xl p-6 w-full space-y-4 font-sans text-gray-800 shadow-sm">
      <div className="flex flex-wrap justify-between items-center gap-4 border-b border-gray-100 pb-4">
        <div>
          <h2 className="text-base font-bold text-gray-900 uppercase tracking-wide">Compiled HTML Production Bundle</h2>
          <p className="text-xs text-gray-500 mt-0.5">This standalone HTML package builds clean cross-platform matching styles layout strings smoothly.</p>
        </div>
        <div className="flex space-x-2 shrink-0">
          <button 
            onClick={handleCopy} 
            className="bg-orange-500 hover:bg-orange-600 text-white font-bold px-4 py-2 text-xs rounded-xl shadow-xs transition duration-150"
          >
            {copied ? 'Copied to Clipboard!' : 'Copy Template Code'}
          </button>
          <button 
            onClick={handleDownload} 
            className="bg-gray-100 hover:bg-gray-200 text-gray-700 font-bold px-4 py-2 text-xs rounded-xl border border-gray-200 transition duration-150"
          >
            Download Raw (.html)
          </button>
        </div>
      ...
      <div className="relative">
        <pre className="bg-gray-50 text-gray-700 text-xs p-4 rounded-xl overflow-x-auto max-h-[480px] font-mono border border-gray-200 selection:bg-orange-100">
          {compileHTML()}
        </pre>
      </div>
    </div>
